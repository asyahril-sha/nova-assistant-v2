...
class MemoryType(Enum):
    ...
    InnerThought = "inner_thought"  # Pikiran dalam hati
...
class Constants:
    ...
    STAGE_DESCRIPTIONS = {
        ...
        IntimacyStage.SoulBonded: "Satu jiwa. Sudah seperti belahan jiwa.",
...
"""
        ...
        try:
            ...
            self.hippocampus.add_memory(
                ...
                memory_type=MemoryType.InnerThought,
                importance=0.6,
...
class EmotionalIntelligence:
    ...
    MOOD_TRANSITIONS = {
        ...
        Mood.Dominan: [Mood.Horny, Mood.Marah, Mood.Posesif, Mood.Agresif],
    ...
    def get_mood_from_context(self, 
        ...
        if location:
            if "kamar" in location.lower():
                ...
                return random.choice([Mood.Ceria, Mood.Rileks])
...
class DominanceSystem:
    ...
    def set_level(self, level: Union[str, DominanceLevel]) -> bool:
        ...
        if isinstance(level, str):
            ...
            level_map = {
                ...
                "sangat dominan": DominanceLevel.VeryDominant,
        ...
        if random.random() < chance:
            if arousal > 0.8 and self.current_level == DominanceLevel.Normal:
                ...
                if random.random() < 0.3:  # 30% chance jadi sangat dominan
                    self.set_level(DominanceLevel.VeryDominant)
            elif arousal > 0.95 and self.current_level == DominanceLevel.VeryDominant:
    ...
    def get_suggestion(self, context: Dict) -> Optional[DominanceLevel]:
        ...
        if level >= 9 and arousal > 0.6:
            return DominanceLevel.VeryDominant
    ...
    def generate_response_modifier(self) -> str:
        ...
        modifiers = {
            ...
            DominanceLevel.VeryDominant: "Bersikaplah sangat dominan, kontrol penuh, jangan ragu memberi perintah.",
    ...
    def __init__(self):
        ...
        self.level_up_messages = {
            2: [
                ...
                "💫 **Level 2!** Ceritain更多 tentang dirimu dong"
            ],
            3: [
...
class LocationSystem:
    ...
    LOCATIONS = {
        Location.LivingRoom: {
            ...
            "mood_effect": [Mood.Ceria, Mood.Rileks, Mood.Malas]
        },
        Location.Bedroom: {
            "name": "kamar tidur",
            "emoji": "🛏️",
        ...
        Location.Balcony: {
            ...
            "activities": ["lihat pemandangan", "minum kopi", "vaping", "melamun"],
            "clothing_style": "casual",
    ...
    def __init__(self):
        self.current_location: Location = Location.LivingRoom
    ...
    def reset(self):
        ...
        self.current_location = Location.LivingRoom
        self.last_move_time = datetime.now()
        self.location_since = datetime.now()
...
Orgasme: {memory_system.orgasm_count if hasattr(memory_system, 'orgasm_count') else 0}x
...
=== MODE DOMINASI ===
Mode saat ini: {dominance_system.current_level.value if hasattr(dominance_system, 'current_level') else 'normal'}
{pref_modifier}
    ...
    async def dominant_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if level in level_map:
            ...
            responses = {
                ...
                DominanceLevel.VeryDominant: "🔥 Kamu sudah milikku sepenuhnya! Jangan banyak gerak!",
...
# ===================== BAB 10.3: Advanced Features =====================
    ...
    async def _background_thought_processing(self, user_id: int, context_data: Dict):
        ...
        try:
            ...
            initiative = await thought_system.should_speak_now(context_data)
            if initiative:
...
# ===================== BAB 11.2: Activity Detection =====================
        ...
        for area, data in SexualDynamics.SENSITIVE_AREAS.items():
            for keyword in data["keywords"]:
        ...
        if not activity_detected:
            for act, data in SexualDynamics.SEX_ACTIVITIES.items():
                for keyword in data["keywords"]:
                    if keyword in user_message.lower():
        ...
        # Trigger dominan
        for trigger in SexualDynamics.DOMINANCE_TRIGGERS:
            if trigger in msg_lower:
        ...
        # Trigger submissive
        for trigger in SexualDynamics.SUBMISSIVE_TRIGGERS:
            if trigger in msg_lower:
